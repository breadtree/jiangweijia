...
<html>
<head>
<title>Manage ringtone in gift bag</title>
...
//end
    ...
    try {
        ...
        else if (purviewList.get("10-8") == null)  {
...
%>
 ...
 function addMember () {
      ...
      <%if(userday.equalsIgnoreCase("1"))
                  ...
                  <%}%>
      var newurl='giftbagMemberAddEnd.jsp?ringid='+fm.crid.value+'&ringfee='+fm.price.value+'&ringlabel='+fm.ringLabel.value+'&singgername='+fm.ringauthor.value+'&validdate='+fm.validdate.value+'&ringspell='+fm.ringspell.value+userday;
      window.document.location.href = newurl;
   }
 function comeBack () {
     window.parent.location.href = 'giftbagMember.jsp?ringgroup='+<%=ringgroup%>+'&grouplabel='+<%="'"+grouplabel+"'"%>
   }

</script>
<form name="inputForm" method="post" action="giftbagMemberEdit.jsp">
...
%>
        ...
        <tr>
          <td height="22" align="right" valign="center">Ringtone spelling</td>
...
<%
        ...
        sysInfo.add(sysTime + operName + " Exception occurred in adding SP gift bag!");
        sysInfo.add(sysTime + operName + e.toString());
        vet.add(operName + " errors occurred in adding SP gift bag!");
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
<form name="errorForm" method="post" action="error.jsp" target="_parent">
<input type="hidden" name="historyURL" value="giftbagMemberEdit.jsp">
</form>
<script language="javascript">
