...
<%
    ...
    if(ischeckop>0&&grpcheck>0)
        ...
        map.put("RESULT","1");
        map.put("DESCRIPTION"," Add member of "+ringgroup+" is:"+ringid);
...
<html>
<head>
<title>Add member in gift bag of SP</title>
               </head>
               <body>
                 <form name="resultForm" method="post" action="result.jsp">
                   <input type="hidden" name="historyURL" value="giftbagMember.jsp?ringgroup=<%=ringgroup%>&grouplabel=<%=grouplabel%>">
<input type="hidden" name="title" value="Add members of gift bag">
...
<%
             ...
             <html>
               <head>
                 <title>Add members of gift bag</title>
                  <script language="JavaScript">
                 <%if(!mess.trim().equalsIgnoreCase("")){%>
                  window.alert('<%=mess%>');
                  <%}%>
               ...
               <body>
                 <form name="resultForm" method="post" action="result.jsp">
                   <input type="hidden" name="historyURL" value="giftbagMember.jsp?ringgroup=<%=ringgroup%>&grouplabel=<%=grouplabel%>">
                     <input type="hidden" name="title" value="Add member in gift bag of SP">
                       <script language="javascript">
                         document.resultForm.submit();
                         </script>
                         </form>
