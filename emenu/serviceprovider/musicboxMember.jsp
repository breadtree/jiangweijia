...
<html>
<head>
<title>Manage music box of SP</title>
...
<%
    ...
    try {
        ...
        if (operID != null && purviewList.get("10-7") != null) {
          ...
          if (op.equals("del")) {
            optype = 2;
                title = "Delete the ringtone in music box:["+grouplabel+"]";
                map.put("DESCRIPTION","Delete the ringtone in music box:[" + grouplabel+"]");
          }
          if(optype>0){
            ...
            if(sysringgrpcheck.equalsIgnoreCase("0")){
              ...
              for (int i = 0; i < rList.size(); i++) {
            	  hashRes = (Hashtable)rList.get(i);
              if(Integer.parseInt((String)hashRes.get("result")) > 0)
            	  map.put("RESULT","0");
              else 
            	  map.put("RESULT","1");
              }
              ...
              if(rList.size()>0){
                ...
                String backurl="musicboxMember.jsp?ringgroup="+ringgroup+"&grouplabel="+grouplabel;
%>
...
%>
   ...
   function addInfo () {
     window.open('ringGroupAdd.jsp?grouptype=1','ringGroupAdd','width=400, height=240,top='+((screen.height-280)/2)+',left='+((screen.width-400)/2));
   }
...
//     var fm = document.inputForm;
//     fm.ringidadd.value = ringid;
//     fm.op.value = 'add';
     document.location.href = 'musicboxMemberAddEnd.jsp?ringidadd='+ringid+'&op=add';
   }

   function backInfo () {
    document.location.href = 'musicbox.jsp';
   }
...
<form name="inputForm" method="post" action="musicboxMember.jsp">
...
<input type="hidden" name="ringidadd" value="">
...
<table width="440" height="400" border="0" align="center" class="table-style2">
  ...
  <td>
        ...
        <tr >
          <td height="26" colspan="2" align="center" class="text-title" background="image/n-9.gif">Manage <%=musicbox%> of SP:<%=grouplabel%></td>
        </tr>
        <td align="center">
...
<%
        ...
        sysInfo.add(sysTime + operName + "Exception occurred in managing music box of SP");
        sysInfo.add(sysTime + operName + e.toString());
        vet.add("Errors occurred in managing music box of SP");
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
<form name="errorForm" method="post" action="error.jsp">
<input type="hidden" name="historyURL" value="musicboxMember.jsp">
</form>
<script language="javascript">
