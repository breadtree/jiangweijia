...
<%
    ...
    try {
	    ...
	    ywaccess access = new ywaccess();
	   
	    String sertype ="";
		int par = 0;
	
	   String par1 = access.getPara(20240);
	
	   if(!(par1.equals("")))
		{
	      par = Integer.parseInt(par1);
		}

        Vector vet1 = syspara.getUserTypeInfodelr(par);
        String option="";
      ...
      sertype= sysring.getsubservice(par);
	  List vet = null;
            vet = sysring.queryDealerRing(par);
	  System.out.println("***par**"+par+"**sertype****" +sertype+"**vet****" +vet);
      

        if (operID != null && purviewList.get("2-45") != null) 
    	{
           
		  if(sertype.equals("1") && !(vet1.isEmpty()) && !(par1.equals("")))
          {
...
//                rsubindex = oldrsubindex;
            ...
            if(optype>0){
               ...
               rList = sysring.setSysDefaultRingdealer(map1);
               // 准备写操作员日志
...
<input type="hidden" name="historyURL" value="dealermgmt.jsp">
...
<script language="javascript">
  ...
  //          List vet = null;
 //           vet = sysring.queryDealerRing(4);//系统赠送铃音
 %>
<script language="javascript">
...
<form name="inputForm" method="post" action="dealermgmt.jsp">
...
<table width="440" height="400" border="0" align="center" class="table-style2">
  ...
  <td>
        ...
        <tr >
          <td height="26" colspan="2" align="center" class="text-title" background="image/n-9.gif">Dealer Free Content management <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%></td>
        </tr>
        <td align="center">
        ...
        <td height='100%'>
            ...
            <tr>
             ...
             <td><input type="text" name="ringid" value="" maxlength="20" class="input-style1"  ><img src="../image/query.gif" alt="Click here,browse and select"  onMouseOver="this.style.cursor='hand'" onclick="javascript:queryInfo()">
             </td>
            </tr>
            <tr>
                <!--
...
<%
   //     }
		...
		{ %>
		<script language="javascript">
		 alert("Dealer User type not configured, please contact Admin");
