...
<html>
<head>
<title>Manage Short Ring ID</title>
...
<%
    ...
    try {
        ...
        if (operID != null && purviewList.get("2-53") != null) {
            ...
            if (op.equals("add")){
                optype = 1;
                title = "Add IVR Short Ring ID";
            }
            else if (op.equals("del")) {
                optype = 2;
                title = "Delete IVR Short Ring ID";
            }
            else if (op.equals("edit")) {
                optype = 3;
                title = "Modify IVR Short Ring ID";
            }
...
<input type="hidden" name="historyURL" value="shortRingID.jsp">
...
%>
   ...
   function checkInfo () {
      ...
      if(value==''){
         alert("Please enter the serial number of a <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%>!");
         fm.rsubindex.focus();
         return flag;
      }
      if (!checkstring('0123456789',value)) {
         alert('The <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> No. must be a digital number!');
         fm.rsubindex.focus();
         return flag;
      }
      flag = true;
   ...
   function addInfo () {
     window.open('shortRingIDAdd.jsp','shortRingIDAdd','width=400, height=280,top='+((screen.height-280)/2)+',left='+((screen.width-400)/2));
   }
   ...
   function editInfo () {
      ...
      if(fm.infoList.length==0){
         alert("Sorry. You must configure short ring id first!");
         return;
      }
      if(index == -1){
          alert("Please select the short ring to be edited");
          return;
      }
     var editURL = 'shortRingIDMod.jsp?ringid=' +fm.ringid.value +'&ringlabel='+fm.ringlabel.value + '&rsubindex='+fm.oldrsubindex.value;
   ...
   function delInfo () {
      ...
      var index = fm.infoList.selectedIndex;
      if(index == -1){
          alert("Please select the short ring to be deleted");
          return;
      }
     fm.op.value = 'del';
     fm.submit();
...
<form name="inputForm" method="post" action="shortRingID.jsp">
...
<table width="440" height="400" border="0" align="center" class="table-style2">
  ...
  <td>
        ...
        <tr >
          <td height="26" colspan="2" align="center" class="text-title" background="image/n-9.gif">Manage Short Ring ID</td>
        </tr>
        <td align="center">
        ...
        <td height='100%'>
            ...
            <tr>
             <td width="30%" align=right >Short Ring Code</td>
...
<%
        ...
        sysInfo.add(sysTime + operName + " Exception occurred in managing Short Ring ID!");
        sysInfo.add(sysTime + operName + e.toString());
        vet.add("Error occurred in managing Short Ring ID!");
