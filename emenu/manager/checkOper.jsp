...
<%!
    public Vector refuseComment() {
        ...
        vet.add("Refuse this ringtone");//该铃音审核不通过
        vet.add("Invalid ringtone property");//铃音属性配置不正确
...
<%
            ...
            map.put("PARA3","Audit not pass");//审核不通过
...
%>
   ...
   function checkRefuse () {
      fm = document.inputForm;
      if (fm.seqno.value == '') {
         alert('Please select the <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> you want to refuse!');//请先选择要拒绝的铃音!
         return false;
      }
   ...
   function searchInfo () {
      fm = document.inputForm;
      if (!CheckInputStr(fm.qryRingName,'The <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> name for approval')){////待审核Ringtone name
