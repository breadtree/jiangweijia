...
<html>
<head>
<title>Manage month period of time ringtone</title>
...
String userringtype = request.getParameter("userringtype") == null ? "" : request.getParameter("userringtype");
    ...
    try {
      ...
      String strOption = "";
      if (operID != null && purviewList.get("13-12") != null) {
	...
	zxyw50.Purview purview = new zxyw50.Purview();
            if(!purview.CheckOperatorRight(session,"13-12",craccount)){
      ...
      if(op.equals("add")){
         ...
         if(checkLen(timeDecrip,20))
         	throw new Exception("The name of month time ringtone is too long,please re-enter!");//您输入的每月time segmentRingtone name超过长度限制,请您重新输入!
...
//                else
         ...
         sysInfo.add(sysTime + craccount + " add month time ringtone sucessfully!");//新增每月time segment铃音配置成功!
         sDesc = "Add";
         ...
         sysInfo.add(sysTime + craccount + " delete month time ringtone successfully!");//删除每月time segment铃音成功!
         sDesc = "Delete";
...
//                else
         ...
         result1 = SocketPortocol.send(pool,hash);
         sDesc = "Edit";
         sysInfo.add(sysTime + craccount + " edit month time ringtone successfully!");//修改每月time segment铃音配置成功!
      }
      //填写日志
      if(!op.equals("")){
          //zxyw50.Purview purview = new zxyw50.Purview();
...
//        Vector vRingDate = new Vector();
		...
		if(!craccount.equals(""))
		   list = db.getUserTimeList(craccount,4,userringtype);
		listCount = list.size();
		if(list.size()>0){
%>
...
%>
        ...
        s_vStartDay[<%= i + "" %>] = '<%= (String)result.get("startday") %>';
        s_vEndDay[<%= i + "" %>] = '<%= (String)result.get("endday") %>';
...
<form name="inputForm" method="post" action="monthRingEnd.jsp">
...
<table border="0" align="center" height="400" width="90%" class="table-style2" >
  ...
  <table border="0" align="center" class="table-style2">
      <tr >
          <td height="26" colspan=2 align="center" class="text-title" background="../image/n-9.gif">Manage month period of time ringtone--<%= craccount %></td>
      </tr>
      <tr >
      ...
      <tr valign="top">
                  <td width="100%">
                    <table width="100%"  border="0" class="table-style2" cellpadding="2" cellspacing="1" >
                      ...
                      <tr>
                        <td align="right" valign="center"><span title="Start day of month">Start day</span></td>
                        <td valign="center">
                          <select name="startmonthday" size="1" class="input-style5">
                            ...
                            %>
                          </select><span title="End day of month">End day<span>&nbsp;<select name="endmonthday" size="1" class="input-style5">
                            <%
                               for(int j=1;j<=31;j++)
                                  out.println("<option "+(j==31?"selected ":"") +"value="+(j<10?("0"+j):""+j) + ">" + String.valueOf(j) + "</option>" );
                            %>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        <td align="right" valign="center"><span title="Start time of day">From</span></td>
                        <td valign="center">
                      ...
                      <tr>
                        <td align="right" valign="center"><span title="End time of day">To</span></td>
                        <td valign="center">
         ...
         <tr>
          ...
          <table border="0" width="100%" class="table-style2">
            <tr>
              ...
              <tr>
                        <td style="color: #FF0000">1.The month period ring back tone setting enables you to play your special ring back tone within a time period of days you specify.</td>
              </tr>
              <tr>
			  ...
			  <tr>
                <td style="color: #FF0000">4.Month period ring back tones have higher priority than the ring back tone for the specified <%=userringtype.equals("0")?"calling":"called"%> number (group). That is, when a subscriber, whose number has been added as the special <%=userringtype.equals("0")?"calling":"called"%> number or a member in the calling number group, calls you within the time period that you set a special ring back tone for, the period ring back tone, rather than that specified for the specified <%=userringtype.equals("0")?"calling":"called"%> number or <%=userringtype.equals("0")?"calling":"called"%> number group, will be played.</td>
              </tr>
              <tr>
                <td style="color: #FF0000">5.The name of the month period ring back tone must be unique and change is not allowed.</td>
              </tr>
           </table>
          </td>
        </tr>
...
<Script language="Javascript">
   ...
   function addTime () {
      ...
      if(fm.ringID.length == 0){
         alert("You did not order any <%=ringdisplay%>. Please order first!");
         return false;
      }
      if(!checkInput())
         return false;
   ...
   function editTime () {
      ...
      if(pform.selTimeList.length ==0){
        alert("There is no period ring back tone that can be changed.");//没有可供更改的time segment'+ringdisplay+'!'
        return;
      }
      ...
      if(pform.ringID.length == 0){
         alert("You did not order any ringtone. Please order ringtone first.");//您当前没有定购任何    请先定购
         return false;
      }
      if(pform.ringID.selectedIndex ==-1){
        alert("Please select ringtone first!");//请先选择
        return false;
      }
     ...
     if(pform.callingNum.value.length>8)//固定电话
      ...
      if(startmonthday>endmonthday){
        alert("The start day of month can't be later than the end day of month!");//每月开始日不能比结束日晚!
        return;
      }

      var fee=<%= modefee %>;
      if(fee>0 && pform.oldcrid.value != pform.ringID.value){
           if(!confirm("You will be charged "+ fee/<%=ratio%> + " <%=majorcurrency%>.\nAre you sure to change the monthly period ringtone?" ))
              return;
      }
...
//        ret = ret + second;
   ...
   function checkInput(){
      ...
      if (!CheckInputStr(pform.timeDecrip,'Monthly period name'))//time segment名称
         return;
      var temp="<%=  listCount %>";
      var flag = 0;
...
//         }
   ...
   function delTime() {
      ...
      if(fm.selTimeList.length ==0){
        alert("There is period ringtone that can be deleted!");//没有可供删除的time segment
        return;
      }

      if(fm.selTimeList.selectedIndex == -1){
         alert("Please select the period ringtone that you want to deleted!");//请选择需要删除的time segment
         return;
      }

      if (! confirm('Are you sure to delete the period ringtone?'))
         return;
      fm.op.value = 'del';
      fm.submit();
   }
...
<%
        ...
        sysInfo.add(sysTime + craccount + ",Exception occurred in setting mongthly period ringtone!");
        sysInfo.add(sysTime + craccount + e.toString());
        vet.add("Exception occurred in setting mongthly period ringtone!");
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
<form name="errorForm" method="post" action="../error.jsp">
<input type="hidden" name="historyURL" value="manualSvc/monthRing.jsp?userringtype=<%=userringtype%>">
</form>
<script language="javascript">
