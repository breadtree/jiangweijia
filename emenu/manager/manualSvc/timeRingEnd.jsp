...
<%
	...
	zxyw50.Purview purview = new zxyw50.Purview();
            if(!purview.CheckOperatorRight(session,"13-5",craccount)){
...
//                else
         ...
         sysInfo.add(sysTime + craccount + " add time period ringtone successfully!");//新增time segment铃音配置成功
         sDesc = "Add";
         ...
         sysInfo.add(sysTime + craccount + " delete time period ringtone successfully!");//删除time segment铃音成功
         sDesc = "Delete";
...
//                else
         ...
         result1 = SocketPortocol.send(pool,hash);
         sDesc = "Edit";
         sysInfo.add(sysTime + craccount + " modify time period ringtone succussfully!");//修改time segment铃音配置成功!
      }
      //填写日志
      if(!op.equals("")){
          //zxyw50.Purview purview = new zxyw50.Purview();
...
<table border="0" align="center" height="400" width="90%" class="table-style2" >
  ...
  <table border="0" align="center" class="table-style2">
      <tr >
          <td height="26" colspan=2 align="center" class="text-title" background="../image/n-9.gif">Ringtone management on the basis of time period--<%= craccount %></td>
      </tr>
      <tr >
                        ...
                        <td valign="center">
                          <select  name="endHour" class="input-style5">
                            <%
                               ...
                               {
                                 if(j==23)
                                  out.println("<option value="+(j<10?("0"+j):""+j) + " selected>" + String.valueOf(j) + "</option>" );
                                 else
                                  out.println("<option value="+(j<10?("0"+j):""+j) + ">" + String.valueOf(j) + "</option>" );
                                }
                                  %>
...
<%
        ...
        sysInfo.add(sysTime + craccount + "Exception occurred in the ringtone management on the basis of time period!");//用户time segment铃音管理出现异常
...
<input type="hidden" name="historyURL" value="manualSvc/timeRing.jsp?userringtype=<%=userringtype%>">
</form>
<script language="javascript">
