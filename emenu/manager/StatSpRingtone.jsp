...
<%
    ...
    try {
      ...
      else if (purviewList.get("4-9004") == null  || (!purview.CheckOperatorRightAllSerno(session,"4-5"))) {
		...
		if(startday.length()!=0 && endday.length()!=0 && checkflag.length()!=0 ){
			...
			//if(!(sp.equals("*")) && spcode.length()!=0 ){
			 ...
			 //	SpManage spman = new SpManage();
				...
				list = statServer.statSPRingtone(hash);
                ...
                if(op.equals("bakdata")){
                  ...
                  String file_name = XlsNameGenerate.get_xls_filename(startday, endday, "springtone", stat_type);
                  response.setHeader("Content-disposition","inline; filename=" + file_name);
                  out.clear();
                  out.println("<table border='1'>");
                  out.println("<tr><td>Date</td><td>SP code</td>");
                  out.println("<td>ringtones uploaded</td>");
                  out.println("<td>ringtones ordered</td>");
                  out.println("<td>income("+majorcurrency+")</td></tr>");

                  String strfee=null;
                  float fee;
                  String tdInformat = "<td align='center' style='mso-number-format:\"\\@\"'>";
                  long num1=0,num2=0; float num3=0;String tVAL="";
                  for (int i = 0; i <list.size(); i++) {
                    ...
                    String uploadtimes = map.get("uploadtimes").toString();
                    int intuploadtimes = Integer.parseInt(uploadtimes);
                    String ordertimes = map.get("ordertimes").toString();
                    int intordertimes = Integer.parseInt(ordertimes);
                    ...
                    out.println("<td align=center>"+(String)map.get("uploadtimes")+"</td>");
                    out.println("<td align=center>"+(String)map.get("ordertimes")+"</td>");
                    out.println("<td align=center>"+displayFee((String)map.get("income"))+"</td>");
                    out.println("</tr>");
                    num1+=intuploadtimes;
                    num2+=intordertimes;
                    num3+=intincome;
                  ...
                  out.println("<td align=center >"+displayFee(String.valueOf(num3))+"</td>");
                  out.println("</tr>");
...
<form name="InputForm" method="post" action="StatSpRingtone.jsp">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0" class="table-style2" height=500 align="center">
  <tr>
	...
	<tr >
          <td height="30" colspan=5 align="center" class="text-title"  background="image/n-9.gif">Query SP Ringtone Statistic</td>
   </tr>
  ...
  <tr>
    <td align="center" valign="top">
      <table border="0" width="100%" cellspacing="1" cellpadding="1" class="table-style2" >
        <tr  class="tr-ring">
          <td width="15%" height="24" align="center" ><span class="style1">Date</span></td>
          <td width="10%" height="24" align="center"><span class="style1">SP code</span></td>
          <td width="20%" height="24" align="center"><span class="style1">ringtones uploaded</span></td>
          <td width="20%" height="24" align="center"><span class="style1">ringtones ordered</span></td>
          <td width="20%" height="24" align="center"><span class="style1">income(<%=majorcurrency%>)</span></td>
        </tr>
<%
        if(list!=null){
          long num1=0,num2=0; float num3=0;String tVAL="";
		 ...
		 for (int i = thepage*15; i < thepage*15+15 && i<list.size(); i++) {
				...
				out.println("<td align=center>" + uploadtimes + "</td>");
                                num1+=intuploadtimes;
          			out.println("<td align=center>" + ordertimes + "</td>");
                                num2+=intordertimes;
                                out.println("<td align=center>" + displayFee(income) + "</td>");
                                num3+=intincome;
...
<%
        ...
        sysInfo.add(sysTime + operName + " Exception occurred in querying this stat.!");//铃音订购查询过程出现异常
        sysInfo.add(sysTime + operName + e.toString());
        vet.add("Error occurred in querying this stat.!");//铃音订购查询过程出现错误
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
</form>
<form name="errorForm" method="post" action="error.jsp">
<input type="hidden" name="historyURL" value="StatSpRingtone.jsp">
</form>
<script language="javascript">
