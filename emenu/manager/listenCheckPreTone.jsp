...
<%
    ...
    try {
        ...
        String ringURL = sysring.listenCheckPreTone(pool,Integer.parseInt(ringindex),usernumber);
...
<%
        sysInfo.add(sysTime + "Pre-listen verified pretone:" + ringindex);
    }
    catch(Exception e) {
        Vector vet = new Vector();
        sysInfo.add(sysTime + ringindex + "Exception occurred in pre-listening the verified pretone!");//试听审核前导音过程出现异常
        sysInfo.add(e.toString());
        vet.add("Failed to pre-listen this verified pretone!");//试听审核前导音失败!
