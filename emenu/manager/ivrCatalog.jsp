    ...
    try {
      if (operID != null && purviewList.get("2-27") != null) {
        ...
        if (op.equals("add")){
          optype = 1;
         // title = "增加IVR分类铃音";
          title = "Add IVR ringtone class";
        }
        else if (op.equals("del")) {
          optype = 2;
          title = "Delete added IVR ringtone class";//删除增加IVR分类铃音
        }
        else if (op.equals("edit")) {
          optype = 3;
          title = "Edit added IVR ringtone class";//Edit增加IVR分类铃音
        }
        if(optype>0){
          ...
          rList = sysring.setIVRCatlog(map1);
          // 准备写操作员日志
          ...
          if(rList.size()>0){
            ...
            <form name="resultForm" method="post" action="result.jsp">
              <input type="hidden" name="historyURL" value="ivrCatalog.jsp">
        ...
        vet = colorring.searchRingCatalog(libid);
...
//              alert(name);
 ...
 function getRingLibName(id){
   ...
   function addInfo (){
     ...
     window.open('ivrCatalogAdd.jsp','ivrCatalogAdd','width=400, height=280,top='+((screen.height-280)/2)+',left='+((screen.width-400)/2));
   }
   function editInfo (ringid,ringlabel,ringpos,libid) {
     var fm = document.inputForm;
     var editURL = 'ivrCatalogMod.jsp?ringid='+ringid+'&ringlabel='+ringlabel+'&rsubindex='+ringpos+'&libid='+libid;
     window.open(editURL,'ivrCatalogAdd','width=400, height=300,top='+((screen.height-300)/2)+',left='+((screen.width-400)/2));
   }
...
<form name="inputForm" method="post" action="ivrCatalog.jsp">
...
<%
        Vector ringLib = sysring.getIvrCatalog();
%>
  <script language="JavaScript">
  ...
  datasource[0] = root;
   <%
    for(int j= 0;j<ringLib.size();j++){
      ...
      String cataindex = (String)table.get("cataindex");
      String tem = "0";
      if(((String)table.get("rsubindex")).equals("0")){
        ;
      }else{
        if(((String)table.get("thirdindex")).equals("0"))
        {
        for(int k=0;k<ringLib.size();k++){
          Hashtable temHash = (Hashtable)ringLib.get(k);
          if(cataindex.equals((String)temHash.get("cataindex")) && ((String)temHash.get("rsubindex")).equals("0")){
            tem = (String)temHash.get("allindex");
          }
         }
        }
        else
        {
         String secondindex= (String) table.get("rsubindex");
         for(int m=0; m<ringLib.size(); m++)
         {
          Hashtable temHash = (Hashtable)ringLib.get(m);
          if(cataindex.equals((String)temHash.get("cataindex")) && ((String)temHash.get("rsubindex")).equals(secondindex)&& ((String)temHash.get("thirdindex")).equals("0")){
            tem = (String)temHash.get("allindex");
          }
         }
        }
      ...
      %>
      var data = new Array('<%=(String)table.get("allindex")%>','<%= tem%>','<%=(String)table.get("description")%>','1');
      datasource[<%=j+1%>] = data;
    <%}%>
  </script>
<tr>
                ...
                <%if(userday.equalsIgnoreCase("1"))
                 ...
                 <%}%>
                  ...
                  <td height="30" width="40">
                    <div align="center"><font color="#FFFFFF"><span title="Price(<%=majorcurrency%>)">DailyPrice(<%=majorcurrency%>)</span></font></div>
                  </td>
				  <%}
...
%>
	 ...
	 <% } %>
        ...
        <td height="20">
          <div align="center"><font class="font-ring"><img src="../image/edit.gif"   width="15" height="15" alt="Edit" onmouseover="this.style.cursor='hand'" onclick="javascript:editInfo('<%= (String)hash.get("ringid") %>','<%= (String)hash.get("ringlabel") %>','<%= (String)hash.get("ringpos") %>','<%= (String)hash.get("libid") %>')"></font></div></td>
        </tr>
<%
         }
        if (vet.size() > 25) {
...
<%
        ...
        sysInfo.add(sysTime + "exception occourred in management of ivr "+ zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")+" category");//分类铃音管理过程出现异常!
        sysInfo.add(sysTime + e.toString());
        vet.add("Error occourred in management of ivr ringtone category");//ivr分类铃音管理过程出现错误!
