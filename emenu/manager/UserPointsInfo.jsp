...
function queryInfo(){
  ...
  if(usernum.length<6){
   // alert('User number输入不正确!');
    alert('Incorrect user number input!');
    document.inputForm.userNumber.focus();
    return;
  }
  ...
  if(parseInt(usernum.substring(0,1))!=0 && parseInt(usernum.substring(0,2))!=13){
   // alert('请输入区号带0的电话号码!');
    alert("Please input the telephone number with the area code of 0");
    document.inputForm.userNumber.focus();
    return;
  } 
  else if(parseInt(usernum.substring(0,1))!=0 && parseInt(usernum.substring(0,2))==13 && usernum.length<11){
    alert('Please input 11-digit MS number!');
    document.inputForm.userNumber.focus();
    return;
  }
  */
  ...
  if(!checkstring('0123456789',usernum)){
    alert('The number should be in the digit format!');
    return;
  }
...
try{
  ...
  else if (purviewList.get("1-15") == null) {
...
<%
     ...
     sysInfo.add(sysTime + operName + "It is abnormal during the query of user usage!");
...
<input type="hidden" name="historyURL" value="UserPointsInfo.jsp">
</form>
<script language="javascript">
