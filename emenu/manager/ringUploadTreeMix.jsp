...
<script language="javascript">
    function doClick (srcObj,idForm,ringlibid,leaf) {
      ...
      if(leaf==1)
         parent.ringUpload.document.URL = "ringUploadMix.jsp?ringlibid=" + ringlibid;
...
<table width="162" border="0" align="center" cellpadding="0" cellspacing="0" class="table-style2">
  ...
  <tr>
   ...
   <tr>
    <td width="100%" onmouseover="this.style.cursor='hand'">Ringtone library</td>
  </tr>
  <tr style="display:<%=spmix.equals("spmix")?"none":""%>">
    <td width="100%" onclick="javascript:doClick(this,'0',503,1)" onmouseover="this.style.cursor='hand'"><img src="folder/midblk.gif"  >Default <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> library</td>
  </tr>
 <%
 ...
 %>
  <tr>
    <td width="100%" onclick="javascript:doClick(<%= imageid %>,<%= menuid %>,<%= ringlibid %>,<%= isleaf %>)" onmouseover="this.style.cursor='hand'"><img src="folder/midplus.gif" id="<%= imageid %>" ><%= nodeName  %></td>
  </tr>
 <%
      if(vetChild.size()>0){
  %>
 ...
 if(vetGrandChild.size()==0){ %>
           <tr>
            <td width="100%" onClick="javascript:doNodeClick('<%= childRinglibid %>','<%=childLeaf%>')" onmouseover="this.style.cursor='hand'">&nbsp;&nbsp;&nbsp;&nbsp;<img src="folder/midblk.gif">&nbsp;<%=childName%>
           </td>
          </tr>
  <%} else {%>
  <tr>
  ...
  <td id="<%= childmenuid  %>" style="DISPLAY:none">
    <table  width="100%" border="0" cellspacing="0" cellpadding="1" class="table-style2">
      ...
      for(int m=0; m<vetGrandChild.size(); m++ ){
        ...
        %>
        <tr>
            <td width="100%" onClick="javascript:doNodeClick('<%= grandChildRinglibid %>','<%=grandChildLeaf%>')" onmouseover="this.style.cursor='hand'" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="folder/midblk.gif">&nbsp;<%=grandChildName%>
           </td>
        </tr>
        <% }%>
    </table>
