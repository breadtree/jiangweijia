...
<%
    ...
    ringdisplay = zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone");
    try {
        SocketPool pool = (SocketPool)application.getAttribute("SOCKETPOOL");
        String spCode = "";
        if (ringID.length() == 0)
            throw new Exception ("Can't try listen this pretone of ringtone because of loss parameters!");
        ...
        String ringURL   =  sysring.tryListenPreTone(pool,ringIndex,usernumber);
...
<%
        sysInfo.add(sysTime + " try listen preTone of " +  ringdisplay + ",ID:" + ringID);
    }
    catch(Exception e) {
        Vector vet = new Vector();
        sysInfo.add(sysTime + ringID + " Exception occurred in try listening" +  ringdisplay);
        sysInfo.add(e.toString());
        vet.add("Try listen " +  ringdisplay + " failed!");
