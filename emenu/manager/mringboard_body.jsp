...
<% 
    ...
    try {
            if (operID != null && purviewList.get("4-22") != null) {
				sysTime = SocketPortocol.getSysTime() + "--";
				ColorRing  colorring = new ColorRing();
				vet = colorring.ringSortBoard(searchvalue);
				rcount = vet.size();
...
<table width="98%" border="0" cellspacing="1" cellpadding="1" align="center" class="table-style4">
         <tr class="tr-ringlist">
                <td height="10" width="210" align="center">Dail Tune Code</td>
				<td height="30" width="311">Dail Tune Name</td>
				<td height="30" width="191">Artist</td>
				<%if(issupportmultipleprice == 1){%>
				<td height="30" width="115" align="center">Daily Price(<%= majorcurrency %>)</td>
                                       <%}
                                if ("1".equals(isSmartPriceFlag)) {
                                  %>
      			 <td height="30" width="115" align="center">Info</td>
                             <% } 
                            else
                             {%>
				<td height="30" width="115" align="center"> <%if(issupportmultipleprice == 1){%>Monthly<%}%> Price(<%= majorcurrency %>)</td>
                   	<% } %>
				<td height="30" width="117" align="center">Buy times</td>
				<td height="30" width="118" align="center">Preview</td>
		</tr>
<%
       if (rcount == 0 ) { %>
		<tr>
...
%>
		...
		<td height="25" align="center"><%= displayFee((String)hash.get("ringfee2")) %></td><%}%>
	        <% if ("1".equals(isSmartPriceFlag)) { %>
		 <td height="25" align="center"> <img src="../image/info.gif"  height='15'  width='15' alt="Ringtone Information" onMouseOver="this.style.cursor='hand'" onClick="javascript:ringInfo('<%= (String)hash.get("ringid") %>','<%= (String)hash.get("buytimes") %>','<%= (String)hash.get("largesstimes") %>')"></td>
		 <% } else {%>
...
<script language="javascript">
   document.location.href="error.jsp?historyURL=mringboard.jsp";
</script>
<%
    }
%>
