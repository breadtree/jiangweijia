...
<html>
<head>
<title>Edit the latest recommended <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%>s</title>
...
<script language="javascript">
   function checkInfo () {
      ...
      if(value==''){
         alert("Please enter the new <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> code!");//请输入Ringtone code
         fm.newringid.focus();
         return flag;
      }
      if (!checkstring('0123456789',value)) {
         alert('The new <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> code must be a digital number!');//Ringtone code必须是数字
         fm.newringid.focus();
         return flag;
      }
      flag = true;
   ...
   function queryInfo() {
	     var result =  window.showModalDialog('ringSearch2.jsp',window,"scroll:yes;resizable:yes;status:no;dialogLeft:"+((screen.width-560)/2)+";dialogTop:"+((screen.height-700)/2)+";dialogHeight:700px;dialogWidth:600px");
	     if(result){
   ...
   function doSure () {
      ...
      if(fm.ringid.value == fm.newringid.value){
         alert("New <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> code is the same as the old one. Please re-enter!");//新Ringtone code同原Ringtone code相同,请重新输入!
         fm.newringid.focus();
         return;
      }
            ...
            <tr>
              <td align="center" colspan=2  height=40>
               <font class="font"><b><img src="../../image/n-8.gif" width="8" height="8">
                 Edit the latest recommended <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%>s</b></font>
              </td>
            </tr>
            <tr>
             <td  align=right height=25 >&nbsp;&nbsp;Queue number</td>
