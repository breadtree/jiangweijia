...
<%
	...
	String MixtuneRingId = request.getParameter("MixtuneRingId") == null ? "0" : (String)request.getParameter("MixtuneRingId");
    String catRingOrderStat = request.getParameter("catRingOrderStat") == null ? "0" : (String)request.getParameter("catRingOrderStat");
    String sysTime = "";
...
//        String sortby = request.getParameter("sortby") == null ? "buytimes" : (String)request.getParameter("sortby");
        ...
        // add in 4.14.01 if fixed libid
        String fixedringlib = request.getParameter("fixedringlib") == null ? "0" : (String)request.getParameter("fixedringlib");

        if(hidemediatype.equals("1")){
        	if(mediatype!=null && !mediatype.equals("") && !mediatype.equals("2")) {
          mediatype = "1";
        }
        }
...
//      }
   ...
   function confirm (ringid) {
     var iMixtuneRingId='<%=MixtuneRingId%>';
     var catRingOrderStat='<%=catRingOrderStat%>';
     ...
     if(is_ie)  {
		 ...
		 if((catRingOrderStat=='1')){
		 	window.opener.document.inputForm.newringid.value = ringid;
		 }
		 else{
			 if((iMixtuneRingId=='0')){
		     	window.opener.document.inputForm.ringid.value = ringid;
			 }  
			 else if(iMixtuneRingId=='1')
			 {
			 	window.opener.document.inputForm.firstRing.value = ringid;
			 }else if(iMixtuneRingId=='2'){
...
<form name="inputForm" method="post" action="ringSearch2.jsp?oper=<%=oper%>">
<input type="hidden" name="page" value="<%= thepage %>">
<input type="hidden" name="pages" value="<%= pages %>">
<input type="hidden" name="ringlib" value="0">
<input type="hidden" name="fixedringlib" value="<%=fixedringlib%>">
<input type="hidden" name="MixtuneRingId" value="<%=MixtuneRingId%>">
<input type="hidden" name="catRingOrderStat" value="<%=catRingOrderStat%>">
<table width="98%" border="0" cellspacing="0" cellpadding="0" align="center">
<tr >
          <td height="26" colspan="5" align="center" class="text-title" background="image/n-9.gif">SP Call Tone</td>
        </tr>
        <tr >
    ...
    <table border="0" cellspacing="1" cellpadding="1" class="table-style2">
        ...
        <%}%>
          <td><%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> sort
              <input type="text" name="ringcatalog" readonly value="All <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> sort" maxlength="20" class="input-style7"  onmouseover="this.style.cursor='hand'"
              <%if(fixedringlib.equals("0")){%>
              onclick="javascript:searchCatalog()"
              <%}%> >
          </td>
...
<input type="hidden" name="historyURL" value="ringSearch2.jsp">
</form>
<script language="javascript">
