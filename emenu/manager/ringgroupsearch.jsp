    ...
    try {
     if (operID != null && bAuth) {
        ...
        if(opertype.equals("edit")){
          ...
          String ringgroupprice = request.getParameter("ringgroupprice") == null ? "0" : (String)request.getParameter("ringgroupprice");
String ringgroupvaliddate = request.getParameter("ringgroupvaliddate") == null ? "" : (String)request.getParameter("ringgroupvaliddate");
          ...
          spM.modifyRingGroupPrice(ringgroupid,ringgroupprice,ringgroupvaliddate);
        }
        ...
        {
           vet = new SysRingGrpDataGateway().querySysRingGroup(1,1);// isshow =1
          if(musicvaliddate.trim().equalsIgnoreCase("1"))
             stylevaliddate="(yyyy.mm)";
        }
        else //大礼包
           vet = new SysRingGrpDataGateway().querySysRingGroup(2,1);// isshow =1
...
<script language="javascript">
 ...
 function confirm (ringid) {
  ...
  function collection (ringID) {
   ...
   function edit(ringgroup,price,validdate){
     document.all.item("editid").style.display="";
     document.inputForm.ringgroupid.value=ringgroup;
     document.inputForm.ringgroupprice.value=price;
   ...
   function editInfo(){
     if(document.inputForm.ringgroupid.value==0)
       //alert('请先点Edit按钮选择要Edit的铃音组!');
         alert("Please click the edit button to edit the ringtone group!");

     if(!checkstring('0123456789',document.inputForm.ringgroupprice.value)){
       //alert("价格只能为整数字符串,请重新输入!");
       alert("Price can only be a digital string ,please re-enter!");
       document.inputForm.ringgroupprice.focus();
       return false;
     }
      <%if("1".equalsIgnoreCase(musicprice.trim())){%>
          var vprice = trim(document.inputForm.ringgroupprice.value);
...
<%
        ...
        if("1".equals(grouptype))
           pageContext.include("musicbox_searchtable.jsp");
        else
           pageContext.include("giftbox_searchtable.jsp");
        if (ringsize > records) {
%>
...
<table class="table-style2"  id="editid" style="display:none">
  ...
  <tr>
      <td>Code <input type="text" name="ringgroupid" readonly value="0" size="15"></td>
      <td>Price(<%=majorcurrency%>)<input type="text" name="ringgroupprice" value="0" maxlength="5" size="10"></td>
      <td>Validdate<%=stylevaliddate%><input type="text" name="ringgroupvaliddate" value="" maxlength="10" size="10"></td>
  </tr>
  <tr>
