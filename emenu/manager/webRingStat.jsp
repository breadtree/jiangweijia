...
<%
    ...
    try {
        ...
        if (purviewList.get("4-13") == null  || (!purview.CheckOperatorRightAllSerno(session,"4-13"))) {
       ...
       if(flag){
          ...
          if(op.equals("search") || op.equals("bakdata")){
             ...
             arraylist = manstat.getWebRingStat(map);
             total     = manstat.getWebRingStat0(map);
          }
	 if(op.equals("bakdata")){
    		response.setContentType("application/msexcel");
            String file_name = XlsNameGenerate.get_xls_filename(startday, endday, "ringOrderMode", XlsNameGenerate.STATISTIC_DAY);
            response.setHeader("Content-disposition","inline; filename=" + file_name);
             	out.clear();
             	out.println("<table border='1'>");
    		out.println("<tr><td>Date</td><td>Total numbers of ring back tone orders</td></tr>");
...
<html>
<head>
<title>Statistics on the ringtone ordering modes</title>
...
<script language="javascript">
   ...
   function modeChange(){
      if(document.forms[0].opermode.value==8){
   ...
   function checkInfo () {
      var fm = document.inputForm;
      var tmp = '';
      if(document.forms[0].opermode.value==8 && document.forms[0].spcode.length==0){
           alert("No ringtone provider can be queried. Please configure SPs first.");//没有铃音供应商可供查询,请先配置SP
           return;
      }
      if (fm.checkTime.checked) {
...
<form name="inputForm" method="post" action="webRingStat.jsp">
...
<script language="JavaScript">
  ...
  <table border="0" width="98%" align="center" cellspacing="0" cellpadding="0" class="table-style2" >
   <tr >
          <td height="26"  align="center" class="text-title"  background="image/n-9.gif">Statistics on the ringtone ordering modes</td>
   </tr>
    <tr>
   ...
   <td align="center">
     ...
     <td  align='center'>
        ...
        <tr>
        <td width="50%"> Ordering modes&nbsp;&nbsp;
         <select name="opermode" class="input-style1"onchange="modeChange();"  style="width:200px" >
	      <option value ="6" >----WEB-----</option>
	      <option value ="7" >----IVR-----</option>
	      <option value ="8" >-----SP-----</option>
	      <option value ="11">---Manual Operator Position---</option>
	      <option value ="13">----Short message----</option>
              <option value ="14">----Pressed key copy----</option>
              <%if(isCombodia==1){%>
     ...
     <tr>
     <td>
     <input type="checkbox" name="checkTime" value="on"<%= checkflag ? " checked" : "" %>  onclick="onCheckTime()" >Start and End dates:YYYY.MM.DD<br>
      Start
  ...
  <%
     if(op.equals("search")){
	  ...
	  out.println("&nbsp;&nbsp;&nbsp;&nbsp;<font style='color: #FF0000'>Total number of ringtone ordering meeting the criteria:&nbsp;" + total + "</font>");
	  out.println("</td></tr>");
     }
  %>
  <tr>
     ...
     <table width="95%" border="0" cellspacing="1" cellpadding="2" align="center"  class="table-style2" id="<%= pageid %>" style="display:none" >
         <tr class="table-title1" align="center">
          ...
          <td height="30" width="60%" align="center" >
              Total number of ringtone orders
          </td>
        </tr>
 ...
 %>
        ...
        <td width="100%" colspan="10">
          <table border="0" cellspacing="1" cellpadding="1" align="right" class="table-style2">
            <tr>
              <td>Total:<%= arraylist.size() %>,&nbsp;&nbsp;<%= pagecount %>&nbsp;page(s),&nbsp;On Page &nbsp;<%= i+1 %>&nbsp;</td>
...
<%
        ...
        sysInfo.add(sysTime + operName + " Exception occurred in the statistics on the ringtone ordering modes!");//铃音订购方式统计过程出现异常
        sysInfo.add(sysTime + operName + e.toString());
        vet.add("Error occurred in the statistics on the ringtone ordering modes!");//铃音订购方式统计过程出现错误
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
<form name="errorForm" method="post" action="error.jsp">
<input type="hidden" name="historyURL" value="webRingStat.jsp">
</form>
<script language="javascript">
