...
<%
    ...
    try {
        if (operID != null && purviewList.get("2-12") != null) {
...
<form name="inputForm" method="post" action="ringRecover.jsp">
   ...
   <td align="center" colspan=2>
      ...
      <%
         ...
         for(int i=0;i<vetRing.size();i++){
           ...
           rList = sysring.hideRing(ringid,4);
          ...
          if(getResultFlag(rList)){
              ...
              map.put("OPERTYPE","208");
              map.put("RESULT","1");
              map.put("PARA1",ringid);
              map.put("PARA2",vetName.get(i).toString());
              sysInfo.add(sysTime + operName + " recover the hidden ringtone of " + ringid + " successfully!");
              purview.writeLog(map);
           } else
              sysInfo.add(sysTime + operName + " recover the hidden ringtone of " + ringid + " failure!");
        }
      %>
...
<%
        ...
        vet = sysring.searchHideRing(hash1);
...
//         alert("qw");
 ...
 function getRingLibName(id){
     ...
     return "Total sort of <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%>";
 }
...
var valueStr = trim(fm.searchvalue.value);
          ...
          if(num1>=0||num2>=0||num3>=0||num4>=0||num5>=0||num6>=0){
            //alert('不能输入单引号或双引号!');
             alert("Cannot input the single quotes or double quotes!");
            fm.searchvalue.focus();
            return;
          }
      }
   ...
   function onHide() {
      ...
      if(fm.ringList.value ==''){
          alert("Please choose the <%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone")%> you want to recover!");
          return;
      }
      fm.op.value = 'hide';
      fm.submit();
...
%>
 ...
 <tr >
          <td height="26"  align="center" class="text-title" background="image/n-9.gif">Recover hidden ring</td>
</tr>
<tr>
    <td width="100%">
    <table  width="98%" border="0" cellspacing="1" cellpadding="1" class="table-style2">
        <tr>
          ...
          <input type="text" name="searchvalue" value="<%= searchvalue %>" maxlength="30" class="input-style2" >
          </td>
...
%>
	...
	<% } %>
        <td height="20">
          <div align="center"><font class="font-ring""><img src="<%=strPhoto%>" height='15'  width='15' alt="Try-listen this ringtone" onmouseover="this.style.cursor='hand'" onClick="javascript:tryListen('<%= (String)hash.get("ringid") %>','<%= (String)hash.get("ringlabel") %>','<%= (String)hash.get("ringauthor") %>','<%= (String)hash.get("mediatype") %>')"></font></div></td>
        </tr>
<%
         }
         ...
         <tr bgcolor="FFFFFF" >
           <td align="center" colspan="9"  height="40"  width="100%">
              ...
              <tr>
               ...
               <img src="button/revert.gif" alt="Recover the hidden ringtone" onmouseover="this.style.cursor='hand'" onClick="javascript:onHide()" > </td>
              </tr>
              </table>
           </td>
         </tr>
...
<%
        ...
        Vector vet = new Vector();
        sysInfo.add(sysTime + " Exception occurred in recovering the hidden ringtone!");
        sysInfo.add(sysTime + e.toString());
        vet.add("Error occurred in recovering the hidden ringtone!");
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
</form>
<form name="errorForm" method="post" action="error.jsp" >
<input type="hidden" name="historyURL" value="ringRecover.jsp">
</form>
<script language="javascript">
