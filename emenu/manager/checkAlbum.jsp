...
<%!
    public Vector refuseComment() {
        ...
        vet.add("The Album audit don't pass");
        vet.add("The Album property config aren't exactly");
        vet.add("The Album name has already exist");
        vet.add("The Album price is unreasonable");
...
<%
    ...
    try {
        ...
        if(operflag ==0 || operID==null) {
            ...
            String    title = "The album audit";
            Hashtable tTmp = new Hashtable();
            if (op.equals("1")) {    
                 rList = sysring.checkMovieAlbum(ringindex,"4","1","Audit pass");
            } else if (op.equals("2")) { 
                 rList = sysring.checkMovieAlbum(ringindex,"5","1",refusecomment);
            }
            if(rList.size()>0) {
              if(getResultFlag(rList)) {
				...
				<input type="hidden" name="historyURL" value="checkAlbum.jsp">
...
%>
      ...
      //opflag: 1:Í¨¹ý,2:¾Ü¾ø
       ...
       switch(opflag){
        case 1:
          ...
          break;
        case 2:
           strcon = "Please select the ringtone group your refuse to pass the audit."; 
   ...
   function checkviews() {
     ...
     if(fm.ringindex.value == ''){
       ...
       var result =  window.showModalDialog('checkAlbumMovieDetail.jsp?groupid='+groupid+'&type=1',window,"scroll:yes;resizable:yes;status:no;dialogLeft:"+((screen.width-460)/2)+";dialogTop:"+((screen.height-300)/2)+";dialogHeight:300px;dialogWidth:560px");
     }
   }
   ...
   function checkRefuse () {
      fm = document.inputForm;
      if (fm.ringindex.value == '') {
         alert('Please select the ringtone you want to refuse.');
         return false;
      }
...
<form name="inputForm" method="post" action="checkAlbum.jsp">
  ...
  <tr valign="center">
    <td>
      <table border="0" align="center" cellspacing="1" cellpadding="1" class="table-style2" width="100%">
        <tr>
          <td height="26" colspan="2" align="center" class="text-title" background="image/n-9.gif">Album audit</td>
        </tr>
        <tr>
        ...
        <td colspan=2>
		    <table border="0" align="center" cellspacing="1" cellpadding="1" class="table-style2" width="100%">
               <tr>
                 <td align="right">The Album name for approval</td>
...
%>
          ...
          <td width="65%">
              <table border="0"  cellspacing="1" cellpadding="1" class="table-style2" width="100%">
               <tr>
                 <td height="22" align="right"><span title="Album name">Name</span></td>
                 <td height="22"><input type="text" name="ringlabel" disabled style="input-style1"></td>
               </tr>
               <tr style="display:none">
                 <td height="22" align="right"><span title="Album provider">Provider</span></td>
                 <td height="22"><input type="text" name="ringsource" disabled style="input-style1"></td>
               </tr>
               <tr>
                 <td height="22" align="right"><span title="Album period of validity">Validity</span></td>
...
<%
      ...
      e.printStackTrace();
        Vector vet = new Vector();
        sysInfo.add(sysTime + operName + " exception occoured in the process of Album audit!");
        sysInfo.add(sysTime + operName + e.toString());
        vet.add(operName + "Error occoured in the process of Album audit!");
