...
%>
<script language="javascript">
  alert('Sorry,you have no access to this group information!');//抱歉,您没有权限访问此集团信息!
</script>
<%
            }
        }
...
function numbersOnly(field,event){
  ...
  if((key == null) || (key == 0) || (key == 8)|| (key == 9) || (key == 13) || (key == 27)){
    ...
    alert('Please input a digital number');//请输入数字
    return false;
  }
}
//查询集团信息
...
<%
        ...
        sysInfo.add(sysTime + " error occourred in the query of group information!");//查询集团信息过程出现错误!
        sysInfo.add(sysTime + e.toString());
        //vet.add("查询集团信息过程出现错误!");
