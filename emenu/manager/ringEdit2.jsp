...
<%
    ...
    try {
            if (operID != null && purviewList.get("2-21") != null) {
...
//       var libid = document.inputForm.ringlib.value;
	 ...
	 <% if (ringablealias1.equalsIgnoreCase("1") || ringablealias2.equalsIgnoreCase("1") || sysfunction.get("2-66-0")== null ) { %>
		var result =  window.showModalDialog('ringInfo.jsp?libid='+libid+'&ringid=' + ringid,window,"scroll:auto;resizable:no;status:no;dialogLeft:"+((screen.width-410)/2)+";dialogTop:"+((screen.height-450)/2)+";dialogHeight:480px;dialogWidth:410px");
	 	 <% } else { %>
     var result =  window.showModalDialog('ringInfo.jsp?libid='+libid+'&ringid=' + ringid,window,"scroll:no;resizable:no;status:no;dialogLeft:"+((screen.width-410)/2)+";dialogTop:"+((screen.height-250)/2)+";dialogHeight:300px;dialogWidth:410px");
		 <% } %>
...
//       var libid = document.inputForm.ringlib.value;
     var result =  window.showModalDialog('delRing.jsp?libid='+libid+'&ringid=' + ringid+'&ringlabel='+ringlabel,window,"scroll:no;resizable:no;status:no;dialogLeft:"+((screen.width-410)/2)+";dialogTop:"+((screen.height-250)/2)+";dialogHeight:300px;dialogWidth:410px");
     if(result&&(result=='yes')){
         var thispage = parseInt(document.inputForm.page.value);
         toPage(thispage);
     }
...
<form name="inputForm" method="post" action="ringEdit2.jsp">
        ...
        <tr>
		  ...
		  <td>
            ...
            <select size="1" name="searchkey" class="select-style5"  onchange="modeChange();" style="width:120px">
              ...
              <option value="ringlabel"><%=zte.zxyw50.util.CrbtUtil.getConfig("ringdisplay","ringtone") +" "+ ring_name%></option>
              <option value="singgername"><%=CrbtUtil.getConfig("ringdisplay","ringtone")%></option>
         ...
         <tr class="tr-ringlist">
                ...
                <%if(userday.equalsIgnoreCase("1"))
                  ...
                  <td height="30" width="50" >
                    <div align="center"><font color="#FFFFFF"><span title="Subscriber period of validity(days)">User Validity</span></font></div>
                  </td>
                 <%}%>
                  <td height="30" width="50" >
...
<%
        ...
        sysInfo.add(sysTime + "Exception occourred in the management of system ringtone!");// 系统铃音管理过程出现异常!
        sysInfo.add(sysTime + e.toString());
       // vet.add("系统铃音管理过程出现错误!");
        vet.add("Error occourred in the management of system ringtone!");
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
<form name="errorForm" method="post" action="error.jsp" >
<input type="hidden" name="historyURL" value="ringEdit2.jsp">
</form>
<script language="javascript">
