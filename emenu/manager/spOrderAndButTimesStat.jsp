...
<%
    ...
    try {
      ...
      else if (purviewList.get("4-75") == null  || (!purview.CheckOperatorRightAllSerno(session,"4-75"))) {
		...
		if(request.getParameter("checkflag") != null && !scp.equals(""))
			...
			if(!(sp.equals("*")) && spcode.length()!=0 ){
                ...
                if(op.equals("bakdata")){
                  ...
                  String file_name = XlsNameGenerate.get_xls_filename(startday, endday, "sptoneorder", stat_type);
                  response.setHeader("Content-disposition","inline; filename=" + file_name);
                  out.clear();
                  out.println("<table border='1'>");
                  out.println("<tr><td>Date</td><td>SP code</td><td>Number of ringtone downloads</td>");
                  if(!(disLargess.equals("1")))
                  out.println("<td >Number of ringtone presents</td>");
                  out.println("<td >Total charge ("+majorcurrency+")</td></tr>");
                  String strfee=null;
                  float fee;
                  String tdInformat = "<td align='center' style='mso-number-format:\"\\@\"'>";
                  for (int i = 0; i <list.size(); i++) {
                    map = (HashMap)list.get(i);
                    String trueBuytimes = String.valueOf(Integer.parseInt(map.get("buytimes").toString())-Integer.parseInt(map.get("prestimes").toString()));
                    out.println("<tr><td>&nbsp;"+(String)map.get("statdate")+"</td>"+tdInformat+map.get("spcode").toString()+"</td><td align=center>"+trueBuytimes+"</td>");
                    if(!(disLargess.equals("1")))
                    out.println("<td align=center>"+(String)map.get("prestimes")+"</td>");
                    strfee = (String)map.get("buyfee");
                    if(strfee.length()==0) {
	...
	//   fm.checkflag.value = 2;
     ...
     //   fm.month.value ="1";
         ...
         //  fm.hour.value ="1";
     	 ...
     	 else if(fm.checkflag[3].checked){//周
...
<form name="InputForm" method="post" action="spOrderAndButTimesStat.jsp">
...
<table border="0" width="100%" cellspacing="0" cellpadding="0" class="table-style2" height=500 align="center">
  <tr>
	...
	<tr >
          <td height="30" colspan=5 align="center" class="text-title"  background="image/n-9.gif">Query SP ringtone detail monthly</td>
   </tr>
   ...
   <td>
    <input type="radio" name="checkflag" value="1" onclick="onCheckTime()">Statistic by month</input></td>
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="0" class="table-style2">
       <td width="80">
         Start date<br>
         <input type="text" name="startday" value="<%= startday %>" maxlength="20" size="20">
				</td>
       <td width="80">End date<br>
       <input type="text" name="endday" value="<%= endday %>" maxlength="20" size="20">
				</td>
<td   align="right" width="40">SP List<br>
...
%>
		  ...
		  <td width="11%" align="right">&nbsp;<br><img border="0" src="button/search.gif" onmouseover="this.style.cursor='hand'" onclick="javascript:searchInfo()">
		  <!-- 
  ...
  <tr>
    <td align="center" valign="top">
      <table border="0" width="100%" cellspacing="1" cellpadding="1" class="table-style2" >
        <tr  class="tr-ring">
          <td width="25%" height="24" align="center" ><span class="style1">Date</span></td>
          <td width="25%" height="24" align="center" ><span class="style1">Ringtone code</span></td>
          <td width="25%" height="24" align="center"><span class="style1">Ringtone name</span></td>
		  <td width="20%" height="24" align="center"><span class="style1">SP name</span></td>
        <% //if(!(disLargess.equals("1"))){%>
          <td width="10%" height="24" align="center"><span class="style1">price(Kyats)</span></td>
        <%//}%>
		  <td width="10%" height="24" align="center"><span class="style1">Buy times</span></td>
		  <td width="10%" height="24" align="center"><span class="style1">Gift times</span></td>
        </tr>
<%
        if(list != null){
		 ...
		 for (int i = thepage*15; i < thepage*15+15 && i<list.size(); i++) {
				...
				out.println("<td align=center>" + (String)map.get("spname") + "</td>");
				out.println("<td align=center>" + (String)map.get("ringfee") + "</td>");
				out.println("<td align=center>" + (String)map.get("buytimes") + "</td>");
				out.println("<td align=center>" + (String)map.get("presenttimes") + "</td>");
          		out.println("</tr>");
		  }
		  %>
  ...
  <tr>
    <td width="100%" colspan="5">
      <table border="0" cellspacing="1" cellpadding="1" align="right" class="table-style2">
        <tr>
          <td>&nbsp;Total:<%= list.size() %>,&nbsp;<%= (list.size() - 1)/15+1 %> page(s),&nbsp;&nbsp;Now on Page&nbsp;<%= thepage + 1 %>&nbsp;</td>
...
<%
        ...
        sysInfo.add(sysTime + operName + " Exception occurred in querying this order!");
        sysInfo.add(sysTime + operName + e.toString());
        vet.add("Error occurred in querying this order!");
        vet.add(e.getMessage());
        session.setAttribute("ERRORMESSAGE",vet);
%>
<form name="errorForm" method="post" action="error.jsp">
<input type="hidden" name="historyURL" value="spOrderAndButTimesStat.jsp">
</form>
<script language="javascript">
