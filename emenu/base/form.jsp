...
function Form(aName){
    ...
    this.turnTo = function(sequence){
        if(sequence<0||sequence>=this.count){
            showMessage("<i18n:message key='UserMSG0055100' />");//不合法的记录笔数
        }
        var url = MODULE_NAME+
    ...
    this.remove = function(){
        ...
        if(state == 'new'){
            ...
            showMessage("<i18n:message key='UserMSG0055101' />");
             return;
        }
    ...
    this.getFieldIndex = function(name){
       ...
       showMessage("<i18n:message key='UserMSG0055102' />"+name+" <i18n:message key='UserMSG0055103' />");
       return -1;
    }
    ...
    this.first = function(){
        if(this.count<=0||this.current<0){
            showMessage("<i18n:message key='UserMSG0055104' />");//没有数据浏览
            return;
        }
        if(this.current==0){
            showMessage("<i18n:message key='UserMSG0055105' />");//当前已经是第一笔数据
            return;
        }
        this.turnTo(0);
    }
    ...
    this.previous = function(){
        if(this.count<=0||this.current<0){
            showMessage("<i18n:message key='UserMSG0055106' />");//没有数据浏览
            return;
        }
        if(this.current==0){
            showMessage("<i18n:message key='UserMSG0055107' />");//当前已经是第一笔数据
            return;
        }
    ...
    this.next = function(){
        if(this.count<=0||this.current<0){
            showMessage("<i18n:message key='UserMSG0055108' />");//没有数据浏览
            return;
        }
        if(this.current==this.count-1){
            showMessage("<i18n:message key='UserMSG0055109' />");//当前已经是最后一笔数据
            return;
        }
        this.turnTo(this.current+1);
    }
    ...
    this.last = function(){
        if(this.count<=0||this.current<0){
            showMessage("<i18n:message key='UserMSG0055110' />");//没有数据浏览
            return;
        }
        if(this.current==this.count-1){
            showMessage("<i18n:message key='UserMSG0055111' />");//当前已经是最后一笔数据
            return;
        }
    ...
    this.save = function(url){
       ...
       if(!v)
        ...
        if(state == 'browser'){
             ...
             showMessage("<i18n:message key='UserMSG0055112' />");
             return;
        }
        var list = new Array();
        list.push(this);
    ...
    this.validity = function(){
        ...
        for(var i=0;i<this.fields.length;i++){
            ...
            if(nullable=='false'){
                if(!value||(value=='')){
                    message+="\n"+item.label+" <i18n:message key='UserMSG0055113' />";//不能为空
                }
            }
