...
function checkIPList(sendObj)
 {
    ...
    for(var ipi=0;ipi<ipArray.length;ipi++){
       if(!checkVarIP(ipArray[ipi])){
            alert("<i18n:message key='UserMSG0055400' />");//地址不合法
            sendObj.select();
			sendObj.focus();
			return false;
       }
       ...
       for(ipj=ipi;ipj<(ipArray.length-1);ipj++){
            ...
            if(ipArray[ipi]==ipArray[ipj+1]){
                alert("<i18n:message key='UserMSG0055401' />");//输入相同的IP地址,不合法!
                sendObj.select();
			    sendObj.focus();
			    return false;
            }
 ...
 function getMonthDays(year,month){
  ...
  function leftTrimt0 (str) {
   ...
   function isUserNumber(usernumber,numbername){
      ...
      if(usernumber==''){
         alert(numbername+" <i18n:message key='UserMSG0055407' />");//,please input 不能为空,请输入!
         return false;
      }
      if(!checkstring('0123456789',usernumber)){
         alert(numbername+" <i18n:message key='UserMSG0055408' />");//只能为数字字符串,请重新输入
         return false;
      }
