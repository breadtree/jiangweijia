 ...
 * b-16. isMobile(Object,allowNull)  是否为MS number
...
function isDate(obj,allowNull)
{
	...
	if (!ifDigit(obj.value))
	{
		return showMsg("The date is incorrect,it cannot contain non-digtial characters!",obj);//日期有误,不能含有非数字的字符
	}
	else if (slen < 8)
	{
		return showMsg("The date format is illegal,the right format is :YYYYMMDD,for example:19990102",obj);//日期格式有误,正确的格式为
	}
	cc = obj.value.substr(0,4);
	if (cc < "1800")
	{
		return showMsg("The year format is illegal,the right format is:YYYY,for example:1999",obj);
	}
	cc = obj.value.substr(4,2);
	if (cc < "01" || cc > "12")
	{
		return showMsg("The month format is illegal,the right format is:MM,for example:01",obj);
	}
	cc = obj.value.substr(6,2);
	if (cc < "01" || cc > "31")
	{
		return showMsg("The date format is illegal,the right format is:DD,for example:02",obj);
	}
	return true;
}
...
function isEmail(obj,allowNull)
{
	...
	if (i == -1 || j == -1 || i > j)
	{
		return showMsg("The email format is incorrect!",obj);//电子邮件书写有误
	}
	return true;
}
...
function isPhone(obj,allwoNull)
{
	...
	for (i=0; i<slen; i++){
		...
		if ((cc <"0" || cc >"9") && cc != "-" && cc!="+" && cc!="(" && cc !=")" && cc !="/")
		{
			return showMsg("Telephone number include illegal character!",obj);//电话号码含有非法字符
		}
	}
	return true;
}
...
function isGBK(obj,allowNull,msgname)
{
	...
	if(slen==0){
		if(!allowNull) 
			return showMsg("Must input " + msgname + "!",obj);
		return true;
	}
	...
	for (i=0; i<slen; i++){
		...
		if (cc.indexOf("%u") >= 0)
		{
			return showMsg( msgname + " cannot be a Chinese character!",obj);
		}
	}
	return true;
}
...
function isMoney(obj,allowNull)
{
	...
	if(slen==0){
		if(!allowNull) 
			return showMsg("Must input!",obj);//必须输入
		return true;
	}
	...
	{
		return showMsg("Not illegal currency value!",obj);//不是合法的货币数
	}	
	return true;
}
...
function isMoneyRange(obj,minValue,maxValue,allowNull)
{
	...
	if(!ifMoney(obj.value))
		return shoMsg("Illegal currency value!",obj);//不是合法的货币数
	
	if(parseFloat(obj.value)>=maxValue)
		return showMsg("The currency value is too large!",obj);
	if(parseFloat(obj.value)<minValue)
		return showMsg("The currency value is too small!",obj);
	return true;
}
...
function checkLeng(obj, min, max,allowNull , msgname )
{
	...
	if (slen < min)
	{
		return showMsg("Please input " + min + " characters at least!",obj);

	}
	if (slen > max)
	{
		return showMsg(msgname + "Please input " + max + " english characters at most,and "+max/2+" chinese characters at least!",obj);
...
function checkValidDate(yyObject,mmObject,ddObject,allowNull){
  	if(allowNull){
  		if(!(((!yyObject.options[0].selected)&&(!mmObject.options[0].selected)&&(!ddObject.options[0].selected)) || ((yyObject.options[0].selected)&&(mmObject.options[0].selected)&&(ddObject.options[0].selected))))
  			return showMsg("The date must all select or none select!",yyObject);
  	}else{
  		if(yyObject.options[0].selected){
  			return showMsg("Please select the year of the date!",yyObject);//请选择日期的年
  		}
  		if(mmObject.options[0].selected){
  			return showMsg("Please select the month of the date!",mmObject);//请选择日期的月
  		}
  		if(ddObject.options[0].selected){
  			return showMsg("Please select the day of the date!",ddObject);//请选择日期的日
  		}
  	}
  	...
  	if(!yyObject.options[0].selected){
  		...
  		if(!checkMonthLength(my_month,my_day,my_year))
  			return showMsg("The selected date is illegal!",ddObject);//选择的日期不合法
  	}
  	return true;
}
...
function checkValidDateRange(yyObject1,mmObject1,ddObject1,allowNull1,yyObject2,mmObject2,ddObject2,allowNull2){
	...
	if((!yyObject1.options[0].selected) && (!yyObject2.options[0].selected)){
		...
		if(date1>date2){
			return showMsg("The start date cannot be larger than the end date!",yyObject1);
		}
	}
	return true;
}
...
function isMobile(obj,allwoNull)
{
	...
	for (i=0; i<slen; i++){
		...
		if ((cc <"0" || cc >"9") && cc != "-" && cc!="+" && cc!="(" && cc !=")" && cc !="/")
		{
			return showMsg("MS number contains illegal character!",obj);
		}
	}
	return true;
}
